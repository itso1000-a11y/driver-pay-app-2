...
const UI_TEXT: Record<Lang, Record<string, string>> = {
  ...
  bg: {
    ...
    nineHourOption: "9ч опция", reducedLimitReached: "лимитът е достигнат", incompleteShift: "Незавършена смяна.", finishBeforeStart: "Крайният час е преди стартовия.", longShift: "Дълга смяна", rest11: "11ч почивка", rest9: "9ч почивка", previousShiftTooLongFor11h: "11ч опция не се показва, защото предишната смяна е над 13ч и се брои като reduced", violation: "Нарушение", pending: "Очаква", backupRestored: "Backup-ът е възстановен.", backupFailed: "Този backup файл не може да се възстанови.", installHelp: "Използвай менюто на браузъра и избери Install app / Add to Home screen.", futureWeekClose: "Затвори празна седмица", fastCloseHint: "Тази седмица няма работни данни. Маркирай дните и я затвори без попълване ден по ден.", allOff: "Всички почивни", allHoliday: "Всички отпуск", chooseDays: "Избор по дни", closeFutureWeek: "Затвори седмицата", goToCurrentWeek: "Към текущата седмица", savedWeeks: "Запазени седмици", noSavedWeeks: "Няма запазени седмици", fullWeek: "пълна", partialWeek: "частична"
  }
};
...
function dayHasEnteredData(day: DayRecord): boolean {
  // KM alone is not work data. A carried-forward start km must not block Off/Holiday.
  return Boolean(day.start || day.finish || day.finishKm || day.holidayPay || day.bonuses.length || day.nightOut || day.splitBreak);
}
...
export default function App() {
  ...
  function setCurrentDayType(type: DayType) {
    ...
    if (type !== "work" && current && dayHasEnteredData(current)) {
      window.alert("Off/Holiday can only be applied to an empty day. Clear the day first if this is not a worked day.");
      return;
    }
    setDays((prev) => prev.map((day, index) => {
      if (index !== currentIndex) return day;
      if (type === "work") return { ...day, dayType: "work" };
      if (type === "holiday") return { ...day, dayType: "holiday", start: "", finish: "", startKm: day.startKm, finishKm: day.finishKm, bonuses: [], nightOut: false, splitBreak: false };
      return { ...day, dayType: "off", start: "", finish: "", holidayPay: "", startKm: day.startKm, finishKm: day.finishKm, bonuses: [], nightOut: false, splitBreak: false };
    }));
  }
  function saveAndGo() {
  ...
  function endWeek(type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) {
    ...
    setArchive((prev) => [{ id: Date.now(), label: weekEndingLabel, createdAt: new Date().toISOString(), days: finalDays, settings, totals: weekTotals, payslip: payslipActualWeek, type }, ...prev]);
