...
type BonusType = "ADR" | "Genset" | "Splitter" | "Driver Assist";
type RestStatus = "good" | "reduced" | "violation" | "unknown";
type DayType = "work" | "off" | "holiday";
type WeekArchiveType = "worked" | "off" | "holiday";
...
type WeekTotals = { worked: number; overtime: number; km: number; taxable: number; untaxed: number; tax: number; ni: number; net: number; total: number };

const BONUS_TYPES: BonusType[] = ["ADR", "Genset", "Splitter", "Driver Assist"];
...
const initialSettings: SettingsState = {
  ...
  nightOutRate: "26.00",
  bonusRates: { ADR: "11.25", Genset: "11.25", Splitter: "11.25", "Driver Assist": "11.25" },
};
...
function fromISODate(iso: string): Date { const [y, m, d] = iso.split("-").map(Number); return new Date(y || 2026, (m || 1) - 1, d || 1); }
function formatDateLabel(date: Date): string { return date.toLocaleDateString("en-GB", { day: "numeric", month: "short" }); }
...
function makeDay(id: string, dayName: string, date: Date): DayRecord {
  return { id, dayName, dateLabel: formatDateLabel(date), dateISO: toISODate(date), start: "", finish: "", startKm: "", finishKm: "", holidayPay: "", dayType: "work", splitBreak: false, nightOut: false, bonuses: [] };
}
...
function getCurrentPayrollSaturdayISO(): string { const today = new Date(); return toISODate(addDays(today, (6 - today.getDay() + 7) % 7)); }
const initialDays = buildPayrollWeek(getCurrentPayrollSaturdayISO());

function sanitizeBonusType(value: unknown): BonusType { return BONUS_TYPES.includes(value as BonusType) ? (value as BonusType) : "ADR"; }
...
function sanitizeDayType(value: unknown): DayType { return value === "holiday" || value === "off" || value === "work" ? value : "work"; }
function sanitizeDayRecord(raw: unknown, fallback: DayRecord): DayRecord {
  const r = (raw ?? {}) as Record<string, unknown>;
  return { ...fallback, id: typeof r.id === "string" ? r.id : fallback.id, dayName: typeof r.dayName === "string" ? r.dayName : fallback.dayName, dateLabel: typeof r.dateLabel === "string" ? r.dateLabel : fallback.dateLabel, dateISO: typeof r.dateISO === "string" ? r.dateISO : fallback.dateISO, start: typeof r.start === "string" ? r.start : "", finish: typeof r.finish === "string" ? r.finish : "", startKm: typeof r.startKm === "string" ? r.startKm : "", finishKm: typeof r.finishKm === "string" ? r.finishKm : "", holidayPay: typeof r.holidayPay === "string" ? r.holidayPay : "", dayType: sanitizeDayType(r.dayType), splitBreak: Boolean(r.splitBreak), nightOut: Boolean(r.nightOut), bonuses: Array.isArray(r.bonuses) ? r.bonuses.map(sanitizeBonusEntry) : [] };
}
function sanitizeSettings(raw: unknown): SettingsState {
  const r = (raw ?? {}) as Record<string, unknown>;
  const br = (r.bonusRates ?? {}) as Record<string, unknown>;
  return { weekdayRate: typeof r.weekdayRate === "string" ? r.weekdayRate : initialSettings.weekdayRate, weekendRate: typeof r.weekendRate === "string" ? r.weekendRate : initialSettings.weekendRate, overtimeThresholdHours: typeof r.overtimeThresholdHours === "string" ? r.overtimeThresholdHours : initialSettings.overtimeThresholdHours, overtimeRate: typeof r.overtimeRate === "string" ? r.overtimeRate : initialSettings.overtimeRate, foodAllowanceRate: typeof r.foodAllowanceRate === "string" ? r.foodAllowanceRate : initialSettings.foodAllowanceRate, nightOutRate: typeof r.nightOutRate === "string" ? r.nightOutRate : initialSettings.nightOutRate, bonusRates: { ADR: typeof br.ADR === "string" ? br.ADR : initialSettings.bonusRates.ADR, Genset: typeof br.Genset === "string" ? br.Genset : initialSettings.bonusRates.Genset, Splitter: typeof br.Splitter === "string" ? br.Splitter : initialSettings.bonusRates.Splitter, "Driver Assist": typeof br["Driver Assist"] === "string" ? br["Driver Assist"] : initialSettings.bonusRates["Driver Assist"] } };
}
function digitsOnly(value: string): string { return value.replace(/\D/g, ""); }
function formatTimeInput(raw: string): string { return digitsOnly(raw).slice(0, 4); }
...
function parseDecimal(value: string): number { const parsed = Number(value.replace(",", ".").trim()); return Number.isFinite(parsed) ? parsed : 0; }
function formatMinutes(value: number | null): string {
  if (value == null) return "—";
  const safe = Math.max(0, Math.round(value));
  return `${Math.floor(safe / 60)}h ${(safe % 60).toString().padStart(2, "0")}m`;
}
function formatMoney(value: number): string { return new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value || 0); }
function isWeekend(dayName: string): boolean { return dayName === "Saturday" || dayName === "Sunday"; }
...
function getOrderedDayIndices(days: DayRecord[]): number[] { return DAY_ORDER.map((id) => days.findIndex((d) => d.id === id)).filter((index) => index !== -1); }
function getPreviousLogicalDay(days: DayRecord[], index: number): DayRecord | undefined { const ordered = getOrderedDayIndices(days); const pos = ordered.indexOf(index); return pos > 0 ? days[ordered[pos - 1]] : undefined; }
function getWeeklyReducedRestCountBeforeIndex(days: DayRecord[], targetIndex: number): number {
  ...
  let count = 0;
  for (let pos = 1; pos < targetPos; pos += 1) if (getBaseRestStatus(getRestBeforeMinutes(days[ordered[pos - 1]], days[ordered[pos]])) === "reduced") count += 1;
  return count;
}
...
function getSuggestedStartTimes(prevFinish: number | null, reducedCount: number) { if (prevFinish == null) return { h11: null, h9: null }; return { h11: prevFinish + 11 * 60, h9: reducedCount < 3 ? prevFinish + 9 * 60 : null }; }
...
function isDayComplete(day: DayRecord): boolean { return (day.dayType === "work" && Boolean(normalizeTime(day.start || "") && normalizeTime(day.finish || ""))) || day.dayType === "off" || day.dayType === "holiday"; }
function getFirstIncompleteIndex(days: DayRecord[]): number { const ordered = getOrderedDayIndices(days); for (const index of ordered) if (!isDayComplete(days[index])) return index; const saturday = days.findIndex((d) => d.id === "sat"); return saturday >= 0 ? saturday : 0; }
function getAdjacentLogicalIndex(days: DayRecord[], currentIndex: number, direction: 1 | -1): number { const ordered = getOrderedDayIndices(days); const currentPos = ordered.indexOf(currentIndex); const nextPos = currentPos + direction; if (currentPos === -1 || nextPos < 0 || nextPos >= ordered.length) return currentIndex; return ordered[nextPos]; }
function getSaturdayDay(days: DayRecord[]): DayRecord { return days.find((day) => day.id === "sat") || days[5] || days[days.length - 1]; }
function getWeekStorageKey(saturdayISO: string): string { return `driverApp_week_${saturdayISO}`; }
function saveWeekData(days: DayRecord[], settings: SettingsState, payslipActualWeek: string) { if (typeof window === "undefined") return; localStorage.setItem(getWeekStorageKey(getSaturdayDay(days).dateISO), JSON.stringify({ days, settings, payslipActualWeek })); }
...
function getLastFinishKmFromPreviousWeek(saturdayISO: string): string {
  ...
  try {
    ...
    for (let i = ordered.length - 1; i >= 0; i -= 1) { const finishKm = previousWeek[ordered[i]]?.finishKm || ""; if (finishKm) return finishKm; }
  } catch { return ""; }
  return "";
}
function getWeekEndingLabel(days: DayRecord[]): string { const saturday = getSaturdayDay(days); const date = fromISODate(saturday.dateISO); return `Tax Week ${getTaxWeekNumber(date)} - ending ${saturday.dateLabel} ${date.getFullYear()}`; }
...
function runDevTests() {
  ...
  console.assert(isDayComplete({ ...makeDay("x", "Monday", new Date()), dayType: "off" }) === true, "off day complete failed");
  console.assert(isDayComplete({ ...makeDay("x", "Monday", new Date()), dayType: "holiday" }) === true, "holiday day complete failed");
...
const buttonStyle: React.CSSProperties = { borderRadius: 14, border: "1px solid #dbe3ee", padding: "12px 14px", fontSize: 15, fontWeight: 600, cursor: "pointer", background: "#fff", transition: "transform 0.08s ease, filter 0.08s ease, background 0.08s ease, opacity 0.08s ease, box-shadow 0.08s ease", WebkitTapHighlightColor: "transparent", userSelect: "none" };
...
export default function App() {
  ...
  const previousWeekFinishKm = !previousDay ? getLastFinishKmFromPreviousWeek(getSaturdayDay(days).dateISO) : "";
  const previousFinishKm = previousDay?.finishKm || previousWeekFinishKm;
  const startKmSuggestionSource = previousDay?.finishKm ? "previous day" : previousWeekFinishKm ? "last week" : "";
  ...
  function saveAndGo() {
    ...
    if (day.dayType === "work") {
      if ((start && !finish) || (!start && finish)) { if (!window.confirm("Incomplete shift. Mark as off day?")) return; }
    ...
    setDays((prev) => prev.map((d, index) => index === currentIndex ? { ...d, start: day.dayType === "work" ? start : "", finish: day.dayType === "work" ? finish : "", startKm: effectiveStartKm } : d));
    setShowBonusForm(false);
    setCurrentIndex(getAdjacentLogicalIndex(days, currentIndex, 1));
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  const computedWeek = useMemo<ComputedDay[]>(() => days.map((day, index) => {
    const previous = getPreviousLogicalDay(days, index);
    const effectiveDay = { ...day, startKm: day.startKm || previous?.finishKm || "" };
  ...
  const weeklyTaxModel = useMemo(() => { const taxable = computedWeek.reduce((sum, day) => sum + day.taxablePay, 0); const untaxed = computedWeek.reduce((sum, day) => sum + day.untaxedPay, 0); const gross = taxable + untaxed; const tax = Math.round(Math.max(0, taxable - 12570 / 52) * 0.2 * 100) / 100; const ni = Math.round((taxable > 242 ? (taxable - 242) * 0.08 : 0) * 100) / 100; return { taxable, untaxed, gross, tax, ni, net: gross - tax - ni }; }, [computedWeek]);
  const taxedWeek = useMemo(() => computedWeek.map((day) => { const share = weeklyTaxModel.taxable > 0 ? day.taxablePay / weeklyTaxModel.taxable : 0; const tax = weeklyTaxModel.tax * share; const ni = weeklyTaxModel.ni * share; return { ...day, tax, ni, net: day.total - tax - ni }; }), [computedWeek, weeklyTaxModel]);
  ...
  const weekBonusSummary = useMemo(() => taxedWeek.reduce((acc, day) => { for (const bonus of day.bonuses) acc[bonus.type] += bonus.qty; if (day.nightOut) acc.nightOuts += 1; return acc; }, { ADR: 0, Genset: 0, Splitter: 0, "Driver Assist": 0, nightOuts: 0 } as Record<BonusType | "nightOuts", number>), [taxedWeek]);
  ...
  return (
    <div style={pageStyle}>
      <style>{`button{transition:transform .08s ease,filter .08s ease,background .08s ease,opacity .08s ease,box-shadow .08s ease;-webkit-tap-highlight-color:transparent;user-select:none}button:active:not(:disabled){transform:scale(.96);filter:brightness(.92)}button:disabled{opacity:.45;cursor:not-allowed}input,select{transition:border-color .12s ease,box-shadow .12s ease,background .12s ease}input:focus,select:focus{border-color:#94a3b8!important;box-shadow:0 0 0 3px rgba(148,163,184,.22)}`}</style>
      <div style={shellStyle}>
        <Header currentDay={currentDay} weekEndingLabel={weekEndingLabel} onWeek={() => setShowWeekView(true)} onSettings={() => setShowSettings(true)} />

        {weekLocked && <div style={{ margin: 16, marginTop: 0, padding: 12, borderRadius: 14, background: "#fff7ed", border: "1px solid #fdba74", color: "#9a3412" }}><div style={{ fontSize: 14, fontWeight: 900 }}>Saved historical week</div><div style={{ fontSize: 12, marginTop: 4 }}>This week is locked to protect old records.</div><button style={{ ...buttonStyle, marginTop: 10, background: "#9a3412", color: "white", borderColor: "#9a3412" }} onClick={() => setHistoricalEditEnabled(true)}>Unlock editing</button></div>}
        ...
        <div style={{ padding: 16, borderTop: "1px solid #eef2f7" }}>
          <div style={{ padding: 12, borderRadius: 14, background: "#f8fafc", border: "1px solid #eef2f7" }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#334155", marginBottom: 8 }}>Week ending Saturday</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}>
              <input style={inputStyle} type="date" value={selectedSaturday} onChange={(e) => setSelectedSaturday(e.target.value)} />
              <button style={buttonStyle} onClick={loadSelectedWeek}>Load</button>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 14 }}>
            <button style={buttonStyle} disabled={currentIndex === orderedIndices[0]} onClick={() => navigateLogical(-1)}>← Prev</button>
            <button style={buttonStyle} disabled={currentIndex === orderedIndices[orderedIndices.length - 1]} onClick={() => navigateLogical(1)}>Next →</button>
          </div>
        </div>

        <div style={sectionStyle}>
          <SectionHeading title="Day type" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
            <DayTypeButton label="Work day" variant="work" active={currentDay.dayType === "work"} onClick={() => setCurrentDayType("work")} />
            <DayTypeButton label="Holiday day" variant="holiday" active={currentDay.dayType === "holiday"} onClick={() => setCurrentDayType("holiday")} />
            <DayTypeButton label="Off day" variant="off" active={currentDay.dayType === "off"} onClick={() => setCurrentDayType("off")} />
          </div>
        </div>

        {currentDay.dayType === "work" && (
          <div style={sectionStyle}>
            <SectionHeading title="Shift" right={currentComputed.weekend ? "Weekend" : undefined} />
            <div style={{ display: "grid", gap: 12 }}>
              <TimeRow label="Start" value={currentDay.start || ""} onChange={(value) => updateTimeValue("start", value)} onBlur={() => normalizeTimeValue("start")} />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <button style={buttonStyle} disabled={suggestedTimes.h11 == null} onClick={() => updateCurrentDay("start", minutesToTime(suggestedTimes.h11))}>Use 11h: {minutesToTime(suggestedTimes.h11) || "—"}</button>
                <button style={buttonStyle} disabled={suggestedTimes.h9 == null} onClick={() => updateCurrentDay("start", minutesToTime(suggestedTimes.h9))}>Use 9h: {minutesToTime(suggestedTimes.h9) || "—"}</button>
              </div>
              <TimeRow label="Finish" value={currentDay.finish || ""} onChange={(value) => updateTimeValue("finish", value)} onBlur={() => normalizeTimeValue("finish")} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
              <MiniStat label="Worked" value={formatMinutes(currentComputed.workedMinutes)} />
              <MiniStat label="OT" value={formatMinutes(currentComputed.overtimeMinutes)} />
            </div>
            {shiftValidationMessage && <div style={{ marginTop: 10, fontSize: 13, fontWeight: 700, color: "#b91c1c" }}>{shiftValidationMessage}</div>}
          </div>
        )}

        <div style={sectionStyle}>
          <SectionHeading title="Kilometres" right={startKmIsSuggested ? "suggested" : undefined} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Field label="Start km">
              <div style={{ position: "relative" }}>
                <input style={{ ...inputStyle, fontSize: 22, fontWeight: 700, textAlign: "center", color: startKmIsSuggested ? "#94a3b8" : "#0f172a", background: startKmIsSuggested ? "#f8fafc" : "#fff", paddingBottom: startKmIsSuggested ? 24 : 12 }} inputMode="numeric" value={displayStartKm} onChange={(e) => updateKmValue("startKm", e.target.value)} placeholder="Start" />
                {startKmIsSuggested && <div style={{ position: "absolute", left: 0, right: 0, bottom: 6, textAlign: "center", fontSize: 10, fontWeight: 800, color: "#64748b", pointerEvents: "none" }}>↳ from {startKmSuggestionSource} finish km</div>}
              </div>
            </Field>
            <Field label="Finish km">
              <input style={{ ...inputStyle, fontSize: 22, fontWeight: 700, textAlign: "center" }} inputMode="numeric" value={currentDay.finishKm || ""} onChange={(e) => updateKmValue("finishKm", e.target.value)} placeholder="Finish" />
            </Field>
          </div>
          <div style={{ marginTop: 8, fontSize: 12, color: "#64748b" }}>{startKmIsSuggested ? `Grey start km is suggested from ${startKmSuggestionSource}. Type over it if it is wrong.` : "Start km can be edited manually."}</div>
          <div style={{ marginTop: 12 }}><MiniStat label="KM run" value={currentComputed.kmRun ?? "—"} /></div>
        </div>

        {currentDay.dayType === "work" && (
          <div style={{ ...sectionStyle, background: restBeforeColors.bg }}>
            <SectionHeading title="Rest from previous day" />
            <RestCard value={formatMinutes(restBeforeMinutes)} colors={restBeforeColors} />
            {currentIndex === orderedIndices[0] && <div style={{ marginTop: 8, fontSize: 12, color: "#64748b" }}>No previous day in this week.</div>}
          </div>
        )}

        {currentDay.dayType === "holiday" && (
          <div style={sectionStyle}>
            <SectionHeading title="Holiday pay" right="taxed" />
            <input style={inputStyle} type="text" inputMode="decimal" value={currentDay.holidayPay || ""} onChange={(e) => updateCurrentDay("holidayPay", e.target.value)} placeholder="0.00" />
          </div>
        )}
        ...
        {currentDay.dayType === "work" && (
          <div style={sectionStyle}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
              <ToggleRow label="Split break" variant="warning" right={hasWeeklySplitBreak ? "Week active" : undefined} value={currentDay.splitBreak} onChange={(checked) => updateCurrentDay("splitBreak", checked)} />
              <ToggleRow label="Night out" variant="success" value={currentDay.nightOut} onChange={(checked) => updateCurrentDay("nightOut", checked)} />
            </div>
          </div>
        )}
        ...
        {currentDay.dayType === "work" && (
          <div style={sectionStyle}>
            <SectionHeading title="Bonuses" />
            {!showBonusForm && <button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={() => setShowBonusForm(true)}>+ Add bonus</button>}
            {showBonusForm && <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 88px", gap: 8 }}><select style={inputStyle} value={draftBonusType} onChange={(e) => setDraftBonusType(sanitizeBonusType(e.target.value))}>{BONUS_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select><input style={inputStyle} inputMode="numeric" value={draftBonusQty} onChange={(e) => setDraftBonusQty(digitsOnly(e.target.value))} /><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={addBonus}>Add</button></div>}
            <div style={{ display: "grid", gap: 8, marginTop: 12 }}>{currentDay.bonuses.length === 0 && <div style={{ fontSize: 14, color: "#64748b" }}>No bonuses added.</div>}{currentDay.bonuses.map((bonus) => <BonusRow key={bonus.id} bonus={bonus} rate={settings.bonusRates[bonus.type]} onDelete={() => removeBonus(bonus.id)} />)}</div>
          </div>
        )}

        <SummarySection currentComputed={currentComputed} />
        <div style={{ ...sectionStyle, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={saveAndGo}>Save & Next</button>
          <button style={buttonStyle} onClick={() => setShowWeekView(true)}>Week View</button>
        </div>
        {showSettings && <SettingsModal settings={settings} setSettings={setSettings} onClose={() => setShowSettings(false)} />}
        {showWeekView && <WeekViewModal weekEndingLabel={weekEndingLabel} weekTotals={weekTotals} payslipActualWeek={payslipActualWeek} setPayslipActualWeek={setPayslipActualWeek} weekDifference={weekDifference} weekBonusSummary={weekBonusSummary} previewWeek={previewWeek} taxedWeek={taxedWeek} setCurrentIndex={setCurrentIndex} close={() => setShowWeekView(false)} endWeek={endWeek} />}
      </div>
    </div>
  );
}
...
function TimeRow({ label, value, onChange, onBlur }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void }) { return <Field label={label}><input style={{ ...inputStyle, fontSize: 28, fontWeight: 900, textAlign: "center", letterSpacing: 1 }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder="00:00" /></Field>; }
...
function ToggleRow({ label, value, onChange, right, variant }: { label: string; value: boolean; onChange: (checked: boolean) => void; right?: string; variant: ToggleVariant }) {
  const palettes: Record<ToggleVariant, { bg: string; border: string; text: string; shadow: string }> = {
    danger: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b", shadow: "inset 0 3px 8px rgba(153,27,27,0.22)" },
    warning: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412", shadow: "inset 0 3px 8px rgba(154,52,18,0.20)" },
    success: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534", shadow: "inset 0 3px 8px rgba(22,101,52,0.20)" },
  };
  const p = palettes[variant];
  const style: React.CSSProperties = value ? { ...buttonStyle, width: "100%", textAlign: "left", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: p.shadow, transform: "translateY(2px)", padding: "13px 14px" } : { ...buttonStyle, width: "100%", textAlign: "left", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "13px 14px" };
  return <button type="button" style={style} onClick={() => onChange(!value)}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}><div><div style={{ fontSize: 15, fontWeight: 900 }}>{label}</div>{right && <div style={{ marginTop: 3, fontSize: 11, fontWeight: 800, opacity: 0.8 }}>{right}</div>}</div><div style={{ fontSize: 13, fontWeight: 950, letterSpacing: 0.6 }}>{value ? "ON" : "OFF"}</div></div></button>;
}

type DayButtonVariant = "work" | "holiday" | "off";
function DayTypeButton({ label, active, onClick, variant }: { label: string; active: boolean; onClick: () => void; variant: DayButtonVariant }) {
  const palettes: Record<DayButtonVariant, { bg: string; border: string; text: string; hint: string }> = {
    work: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534", hint: "hours + extras" },
    holiday: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412", hint: "paid, no shift" },
    off: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b", hint: "no pay" },
  };
  const p = palettes[variant];
  const style: React.CSSProperties = active
    ? { ...buttonStyle, width: "100%", textAlign: "left", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: "inset 0 4px 10px rgba(15,23,42,0.16)", transform: "translateY(2px)", padding: "14px 14px" }
    : { ...buttonStyle, width: "100%", textAlign: "left", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "14px 14px" };
  return <button type="button" style={style} onClick={onClick}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}><div><div style={{ fontSize: 15, fontWeight: 950 }}>{label}</div><div style={{ marginTop: 3, fontSize: 11, fontWeight: 800, opacity: 0.75 }}>{p.hint}</div></div><div style={{ fontSize: 13, fontWeight: 950 }}>{active ? "SELECTED" : "—"}</div></div></button>;
}
...
function SummarySection({ currentComputed }: { currentComputed: ComputedDay }) { return <div style={sectionStyle}><SectionHeading title="Summary" /><Row label="Base pay" value={formatMoney(currentComputed.basePay)} /><Row label="Overtime pay" value={formatMoney(currentComputed.overtimePay)} /><Row label="Bonuses (taxed)" value={formatMoney(currentComputed.bonusPay)} /><Row label="Holiday pay (taxed)" value={formatMoney(currentComputed.holidayPayAmount)} /><Row label="Night out (untaxed)" value={formatMoney(currentComputed.nightOutPay)} /><Row label="Food allowance (untaxed)" value={formatMoney(currentComputed.foodAllowancePay)} /><div style={{ height: 1, background: "#eef2f7", margin: "10px 0" }} /><Row label="Taxable total" value={formatMoney(currentComputed.taxablePay)} /><Row label="Untaxed total" value={formatMoney(currentComputed.untaxedPay)} /><Row label="Gross total" value={formatMoney(currentComputed.total)} strong /><Row label="Tax" value={formatMoney(currentComputed.tax)} /><Row label="NI" value={formatMoney(currentComputed.ni)} /><Row label="Net (after tax)" value={formatMoney(currentComputed.net)} strong /></div>; }
...
function SettingsModal({ settings, setSettings, onClose }: { settings: SettingsState; setSettings: React.Dispatch<React.SetStateAction<SettingsState>>; onClose: () => void }) { return <Overlay onClose={onClose}><ModalCard><ModalTitle>Settings</ModalTitle><SectionHeading title="Pay rates" /><SettingsInput label="Weekday pay rate" value={settings.weekdayRate} onChange={(v) => setSettings({ ...settings, weekdayRate: v })} /><SettingsInput label="Weekend pay rate" value={settings.weekendRate} onChange={(v) => setSettings({ ...settings, weekendRate: v })} /><SettingsInput label="Overtime threshold (hours)" value={settings.overtimeThresholdHours} onChange={(v) => setSettings({ ...settings, overtimeThresholdHours: v })} /><SettingsInput label="Overtime pay rate" value={settings.overtimeRate} onChange={(v) => setSettings({ ...settings, overtimeRate: v })} /><SettingsInput label="Food allowance per worked day" value={settings.foodAllowanceRate} onChange={(v) => setSettings({ ...settings, foodAllowanceRate: v })} /><SettingsInput label="Night out pay" value={settings.nightOutRate} onChange={(v) => setSettings({ ...settings, nightOutRate: v })} /><SectionHeading title="Bonus pay rates" />{BONUS_TYPES.map((bonusType) => <SettingsInput key={bonusType} label={bonusType} value={settings.bonusRates[bonusType]} onChange={(v) => setSettings({ ...settings, bonusRates: { ...settings.bonusRates, [bonusType]: v } })} />)}<button style={{ ...buttonStyle, width: "100%", marginTop: 8, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={onClose}>Done</button></ModalCard></Overlay>; }
function WeekViewModal(props: { weekEndingLabel: string; weekTotals: WeekTotals; payslipActualWeek: string; setPayslipActualWeek: (value: string) => void; weekDifference: number; weekBonusSummary: Record<BonusType | "nightOuts", number>; previewWeek: ComputedDay[]; taxedWeek: ComputedDay[]; setCurrentIndex: (index: number) => void; close: () => void; endWeek: (type: WeekArchiveType) => void }) {
  const p = props;
  const workedStyle = { ...buttonStyle, background: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "1px solid #86efac", color: "#166534", fontWeight: 900 };
  const offStyle = { ...buttonStyle, background: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "1px solid #fca5a5", color: "#b91c1c", fontWeight: 900 };
  const holidayStyle = { ...buttonStyle, background: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "1px solid #fdba74", color: "#9a3412", fontWeight: 900 };
  return <Overlay onClose={p.close}><ModalCard><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}><div><ModalTitle>Week Preview</ModalTitle><div style={{ fontSize: 14, fontWeight: 800, color: "#334155", marginTop: 4 }}>{p.weekEndingLabel}</div></div><button style={{ ...buttonStyle, padding: "8px 12px" }} onClick={p.close}>Close</button></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}><MiniStat label="Hours" value={formatMinutes(p.weekTotals.worked)} /><MiniStat label="KM" value={String(p.weekTotals.km)} /><MiniStat label="Taxable" value={formatMoney(p.weekTotals.taxable)} /><MiniStat label="Untaxed" value={formatMoney(p.weekTotals.untaxed)} /><MiniStat label="Gross" value={formatMoney(p.weekTotals.total)} /><MiniStat label="Est. Net" value={formatMoney(p.weekTotals.net)} /></div><div style={{ marginTop: 12 }}><div style={{ fontSize: 15, fontWeight: 700, color: "#334155", marginBottom: 8 }}>Weekly payslip net</div><input style={{ ...inputStyle, fontSize: 20, fontWeight: 700, textAlign: "center" }} type="text" inputMode="decimal" value={p.payslipActualWeek} onChange={(e) => p.setPayslipActualWeek(e.target.value)} placeholder="0.00" /></div><div style={{ ...getDifferenceStyle(p.weekDifference), borderRadius: 14, padding: 12, marginTop: 12 }}><div style={{ fontSize: 12, marginBottom: 4 }}>Weekly difference</div><div style={{ fontSize: 20, fontWeight: 800 }}>{p.payslipActualWeek ? formatMoney(p.weekDifference) : "—"}</div></div><div style={{ marginTop: 10, padding: 12, borderRadius: 14, border: "1px solid #eef2f7", background: "#f8fafc" }}><div style={{ fontSize: 11, color: "#64748b", marginBottom: 8 }}>Summary</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", rowGap: 6, columnGap: 10 }}><SummaryPair label="Overtime" value={formatMinutes(p.weekTotals.overtime)} /><SummaryPair label="Night outs" value={String(p.weekBonusSummary.nightOuts)} /><SummaryPair label="ADR" value={String(p.weekBonusSummary.ADR)} /><SummaryPair label="Splitter" value={String(p.weekBonusSummary.Splitter)} /><SummaryPair label="Genset" value={String(p.weekBonusSummary.Genset)} /><SummaryPair label="Driver Assist" value={String(p.weekBonusSummary["Driver Assist"])} /></div></div><div style={{ display: "grid", gap: 10, marginTop: 12 }}><button style={workedStyle} onClick={() => p.endWeek("worked")}>End week (worked)</button><button style={offStyle} onClick={() => p.endWeek("off")}>End week (off)</button><button style={holidayStyle} onClick={() => p.endWeek("holiday")}>End week (holiday)</button>{p.previewWeek.map((day) => { const originalIndex = p.taxedWeek.findIndex((d) => d.id === day.id); return <button key={day.id} onClick={() => { p.setCurrentIndex(originalIndex); p.close(); window.scrollTo({ top: 0, behavior: "smooth" }); }} style={{ ...buttonStyle, textAlign: "left", padding: 12 }}><div style={{ fontWeight: 900, fontSize: 18 }}>{day.dayName}</div><div style={{ marginTop: 4, fontSize: 17, fontWeight: 800 }}>{day.dateLabel}</div><div style={{ marginTop: 6, fontSize: 14, color: "#64748b" }}>{day.dayType === "holiday" ? "Holiday" : day.dayType === "off" ? "Off" : `${normalizeTime(day.start) || "—"} → ${normalizeTime(day.finish) || "—"}`}</div><div style={{ marginTop: 8, fontSize: 12 }}>{formatMinutes(day.workedMinutes)} · OT {formatMinutes(day.overtimeMinutes)} · {day.kmRun ?? "—"} km</div><div style={{ marginTop: 8, fontWeight: 900, fontSize: 18 }}>{formatMoney(day.net)} net</div><div style={{ marginTop: 3, fontSize: 12, color: "#64748b" }}>{formatMoney(day.total)} gross</div></button>; })}</div></ModalCard></Overlay>;
}
